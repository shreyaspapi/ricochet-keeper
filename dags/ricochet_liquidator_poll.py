"""
Liquidator Block Poll

- Every 5 minutes check if someone has done the Deposit
- Save the address to sql if not already present
...
SCHEDULE_INTERVAL = Variable.get("liquidator-poll-schedule-interval", "*/15 * * * *")
...
dag = DAG("ricochet_liquidator_poll", catchup=False, default_args=default_args, schedule_interval=SCHEDULE_INTERVAL)
